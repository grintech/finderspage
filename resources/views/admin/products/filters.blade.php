			...
			<div class="dropdown-item">
				<div class="row">
					<div class="col-md-12">
						...
						<select class="form-control" name="category[]" multiple id="category-finder">
					      	<?php foreach($categories as $c): ?>
					      		<?php $subs = $c->sub_categories; ?>
					      		<?php if($subs->count() > 0): ?>
					      			<optgroup label="<?php echo $c->title ?>">
					      				<?php foreach($subs as $c): ?>
					      					<?php $nSubs = $c->sub_categories ?>
					      					<?php if($nSubs->count() > 0): ?>
						      					<?php foreach($nSubs as $nc): ?>
						      						<option value="<?php echo $nc->id ?>" <?php echo isset($_GET['category']) && in_array($nc->id, $_GET['category'])  ? 'selected' : '' ?>><?php echo $c->title . ' > ' . $nc->title ?></option>
						      					<?php endforeach; ?>
						      				<?php else: ?>		
