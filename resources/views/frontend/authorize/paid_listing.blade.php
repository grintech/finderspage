         ...
         <div class="row">
            ...
            @if($type=='fundraiser')
               <div class="col-md-6 img-frame">
                  <img src="{{asset('user_dashboard/img/fundraiser-banner.jpg')}}" alt="Banner Image" class="img-fluid payment-img">
               </div>
            @else
            <div class="col-md-6 img-frame">
               <img src="{{asset('user_dashboard/img/paid-listing_1_new.jpg')}}" alt="Banner Image" class="img-fluid payment-img">
            </div>
            @endif

            <!-- Grid column for the form -->
            <div class="col-md-6 mt-4 mt-md-0" id="sec_new">
               <h1 class="strip-heading">Paid Listing</h1>
               <div class="panel panel-default credit-card-box">
                  <div class="col-md-12">
                     <!-- Image here, but it will be hidden because it's empty -->
                  </div>
                  <div class="panel-heading display-table">
                     <h3 class="panel-title m-title" id="pay_new"> $4.98 per listing </h3>
                  </div>
                  <div class="panel-body w-100">
                     ...
                     @endif
                     <form role="form" action="{{ route('pay.paid-listing.save',['postID' => $post_id,'type'=>$type]) }}" method="post" class="require-validation strip-form" data-cc-on-file="false" data-stripe-publishable-key="" id="payment-form">
                        @csrf
                        <div class="row" id="new_css">
                           <div class="col-lg-6">
                              <div class='form-row'>
                                 ...
                                 <input type="radio" name="planprice" value="{{$post_price}}" class="radioBtnClass " checked="checked">
                                 <label>Paid listing </label>
                                 <span class="showprice"> $ {{$post_price}}/listing </span>
                              </div>
                           </div>
                        </div>
                        <div class="strip-section">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class='form-row'>
                                          ...
                                          <div class="row justify-content-center">
                                             <div class="col-xs-12">
                                                <button class="contact-from-button mb-2" type="submit">Pay Now</button>
                                             </div>
                                          </div> 
                                        </div>
                                    </div>
                           ...
                           {{-- <div class="col-lg-12 text-center mt-3">
                             <a href="{{ route('paypal.payment',['id' => $post_id , 'amt' => $post_price ]) }}">
...
<script>
   var amount = <?php echo $post_price; ?>;
  paypal.Buttons({
    createOrder: function(data, actions) {
      return actions.order.create({
        purchase_units: [{
          amount: {
            value: amount // Replace with your amount
          }
        }]
      });
    },
    onApprove: function(data, actions) {
      ...
      //   alert(post_id);
        ...
        $.ajax({
          url: '/payment-checkout-all-paidpost', // Replace with your route
          method: 'POST',
          data: {
            amount:amount,
            post_id: post_id,
