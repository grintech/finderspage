			...
			<div class="card">
				...
				<div class="card-header">
					<div class="row align-items-center">
						<div class="col-8">
							<h3 class="mb-0">Create New Feedback Here.</h3>
						</div>
					</div>
				</div>
				<div class="card-body">
					<form method="post" action="<?php echo route('admin.feedback.add') ?>" class="form-validation">
						<!--!! CSRF FIELD !!-->
						{{ @csrf_field() }}
						...
						<div class="pl-lg-4">
							<div class="form-group">
								...
								<input type="text" class="form-control" name="category" required placeholder="Category" value="{{ old('category') }}">
								@error('category')
								    <small class="text-danger">{{ $message }}</small>
								@enderror
							</div>
						...
						<div class="pl-lg-4">
							<div class="form-group">
								<label class="form-control-label" for="input-rating">Rating</label>
								<input type="number" class="form-control" name="rating" required placeholder="Rating" value="{{ old('rating') }}">
								@error('rating')
								    <small class="text-danger">{{ $message }}</small>
								@enderror
							</div>
