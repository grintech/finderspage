...
class Testimonials extends AppModel
{
    ...
    /**
    * Testimonials -> Admins belongsTO relation
    * 
    * @return Admins
    */
    ...
    */
    ...
    {
        $record = Testimonials::where('id', $id)
            ->with([
    ...
    * @param $orderBy
    */
    public static function getRow($where = [], $orderBy = 'testimonials.id desc')
    {
        $record = Testimonials::orderByRaw($orderBy);
    ...
    */
    public static function create($data)
    {
        ...
        $testimonials->created_by = AdminAuth::getLoginId();
        $testimonials->created = date('Y-m-d H:i:s');
        $testimonials->modified = date('Y-m-d H:i:s');
        if($testimonials->save())
        {
            if(isset($data['title']) && $data['title'])
            {
                $testimonials->slug = Str::slug($testimonials->title) . '-' . General::encode($testimonials->id);
                $testimonials->save();
            }
    ...
    */
    public static function modifyAll($ids, $data)
    {
        if(!empty($ids))
        {
            return Testimonials::whereIn('testimonials.id', $ids)
                    ->update($data);
        }
        else
        {
