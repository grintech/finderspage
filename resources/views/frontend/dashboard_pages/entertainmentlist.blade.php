      ...
      <div class="container">
        <div class="row justify-content-end">
          <div class="col-lg-12">
            ...
            <h3 class="d-inline"><b>Your Entertainment Listings</b></h3>
            <a class="btn profile-button float-right mb-2" href="{{route('Entertainment')}}">Add</a>
                    ...
                    if(is_array($itemFeaturedImage)) {
                      foreach($itemFeaturedImage as $keyitemFeaturedImage => $valueitemFeaturedImage) { if($keyitemFeaturedImage == 0) {$class='active'; } else { $class='in-active'; } ?>
                          <div class="carousel-item <?= $class; ?>">
                            <img src="{{asset('images_entrtainment')}}/{{ $valueitemFeaturedImage }}" alt="Los Angeles" class="img-fluid d-block w-100" onerror="this.onerror=null; this.src='https://www.finderspage.com/public/images_blog_img/1688636936.jpg';">
                          </div>
                      <?php }     
                    }
                  ?>
                ...
                ?>
                  <div class="caption-frame">
                    ...
                    <p>
                      <b>
                          @php
                            $givenTime = strtotime($blogs->created_at);
                            $currentTimestamp = time();
                            $timeDifference = $currentTimestamp - $givenTime;
                            ...
                            $alertMessage = "";
                              ...
                              if ($days == 44) {
                              echo $alertMessage = "Your free listing expires in 1 day.";
                              }elseif ($days > 44) {
                                echo $alertMessage = "Your free listing has expired.";
                              } else {
                                $remainingDays = 44 - $days;
                                $dayText = $remainingDays == 1 ? "day" : "days";
                              echo $alertMessage = "Your free listing expires in $remainingDays $dayText.";
                              }
                      ...
                      <tbody>
                        ...
                        <tr class="action-list">
                          ...
                          <td colspan="3" class="btn-list">
                            <a href="{{route('Entertainment.single.listing',$blogs->slug)}}" class="btn" data-toggle="tooltip" data-placement="top" title="View"><i class="fa fa-eye" aria-hidden="true"></i></a>
                            
                            <?php 
                            $currentDateTime = new DateTime();
                            $givenTime = $blogs->created_at;
                        ...
                        <!-- @if($userData->subscribedPlan != '7-day-14' && $userData->subscribedPlan != '1-month-55' && !empty($userData->subscribedPlan )) -->
                          <!-- @if($currentDateTime > $nextTenDays)
                            <a href="{{route('Entertainment.d_edit',$blogs->slug)}}" class="btn btn-warning" data-toggle="tooltip" data-placement="top" title="Edit">
                        ...
                        <!-- @else
                         <a href="{{route('pricing')}}" class="btn btn-warning" data-toggle="tooltip" data-placement="top" title="Edit"><i class="far fa-edit"></i></a>
                        @endif -->

                        <a href="{{route('Entertainment.d_edit',$blogs->slug)}}" class="btn btn-warning" data-toggle="tooltip" data-placement="top" title="Edit" >
                              <i class="far fa-edit"></i>
                            </a>
                            <a id="blog_delete" delete-link="{{route('Entertainment.delete',$blogs->id)}}" href="#" class="btn btn-red" data-toggle="tooltip" data-placement="top" title="Delete"><i class="fa fa-trash" aria-hidden="true"></i></a>
                             @if($blogs->bumpPost=="1")
                              ...
                              <a data-toggle="tooltip" data-placement="top" title="You have a free bump" href="{{route('bump.free.success.entertainment',$blogs->id)}}" class="btn btn-primary">Bump</a>
                              ...
                              <a href="{{route('pay.auth.entertainment',General::encrypt($blogs->id))}}" class="btn btn-primary" data-toggle="tooltip" data-placement="top" title="You've used up all your free bump posts from your subscription plan">Bump</i></a>
                              @else
                              <a href="{{route('pay.auth.entertainment',General::encrypt($blogs->id))}}" class="btn btn-primary" data-toggle="tooltip" data-placement="top" title="$5.08 to bump this listing">Bump</i></a>
                            @endif
                            ...
                            <a data-toggle="tooltip" data-placement="top" title="You have an {{ lcfirst($userData->featured_post_count) }} subscription plan" href="{{route('featured.post_count.success.entertainment',$blogs->id)}}" class="btn btn-primary">Feature</a>
                            @else
                              <a data-toggle="tooltip" data-placement="top" title="Get featured! Starting at just ${{$plan_month->price}} for one month." href="{{route('paypal.featured_post.entertainment', ['post_id' => General::encrypt($blogs->id)])}}" class="btn btn-primary" data-toggle="tooltip" data-placement="top" title="Bump">Feature</i></a>
                           @endif
                          </td>
                        </tr>
                      </tbody>
        ...
        var link = $(this).attr("delete-link");
         Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                Swal.fire(
				      'Deleted!',
				      'Your Entertainment listing has been deleted.',
				      'success'
				    )
            }
        });
