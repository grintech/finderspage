	...
	<div class="row">
		<div class="col-xl-12 order-xl-1">
			<div class="card">
				...
				<div class="card-body">
					<form method="post" action="<?php echo route('admin.slider.add') ?>" class="form-validation">
						<!--!! CSRF FIELD !!-->
						{{ @csrf_field() }}
						...
						<div class="pl-lg-4">
							...
							<div class="form-group">
								...
								<div class="custom-control">
									<label class="custom-toggle">
										<input type="checkbox" onchange="$('.btn-section').toggleClass('d-none'); $('.btn-section input').val('');">
										<span class="custom-toggle-slider rounded-circle" data-label-off="OFF" data-label-on="ON"></span>
									</label>
								</div>
							</div>
							<div class="form-group btn-section d-none">
								<label class="form-control-label" for="input-first-name">Button Title</label>
								<input type="text" class="form-control" name="button_title" required placeholder="Button Title" value="{{ old('button_title') }}">
								@error('button_title')
								    <small class="text-danger">{{ $message }}</small>
								@enderror
							</div>
							<div class="form-group btn-section d-none">
								...
								<input type="text" class="form-control" name="button_url" required placeholder="Button Title" value="{{ old('button_url') }}">
								@error('button_url')
								    <small class="text-danger">{{ $message }}</small>
								@enderror
							</div>
